...
export default function LeaderboardsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6">
      <h1 className="mb-6 text-2xl font-bold text-gray-900">{t("leaderboard.title")}</h1>
        ...
        <div className="flex rounded-md bg-gray-100">
          {(["WEEKLY", "MONTHLY"] as LeaderboardPeriod[]).map((p) => (
            <button key={p} onClick={() => setPeriod(p)}
              ...
              {p === "WEEKLY" ? t("leaderboard.weekly") : t("leaderboard.monthly")}
            </button>
          ))}
        </div>
        <div className="flex rounded-md bg-gray-100">
          {(["MOST_POINTS", "LONGEST_STREAK"] as LeaderboardCategory[]).map((c) => (
            <button key={c} onClick={() => setCategory(c)}
              ...
              {c === "MOST_POINTS" ? t("leaderboard.mostPoints") : t("leaderboard.longestStreak")}
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="px-4 py-3 font-medium text-gray-700">{t("leaderboard.rank")}</th>
                <th className="px-4 py-3 font-medium text-gray-700">{t("leaderboard.player")}</th>
                <th className="px-4 py-3 text-right font-medium text-gray-700">{t("leaderboard.score")}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <p className="text-center text-sm text-gray-500">{t("leaderboard.empty")}</p>
      )}
    </div>
  );
}
